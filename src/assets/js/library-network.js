(() => {
  ...
  const links=data.links.map(l=>({...l})),books=nodes.filter(n=>n.type==='book'),themes=nodes.filter(n=>n.type==='theme');
  // Bibliographic and thematic membership also connects books directly.
  // Series membership alone is deliberately not a recommendation.
  const peerPairs=new Map();
  for(const hub of nodes.filter(n=>n.type==='author'||n.type==='theme')){
    const members=links.filter(l=>l.source===hub.id||l.target===hub.id).map(l=>byId.get(l.source===hub.id?l.target:l.source)).filter(n=>n?.type==='book');
    for(let i=0;i<members.length;i++)for(let j=i+1;j<members.length;j++){
      const a=members[i],b=members[j];if(a.language!==b.language)continue;
      const key=[a.id,b.id].sort().join('|');
      if(!peerPairs.has(key))peerPairs.set(key,{source:a.id,target:b.id,type:'connection',reasons:[]});
      peerPairs.get(key).reasons.push((hub.type==='author'?(de?'Gemeinsamer Autor: ':'Same author: '):(de?'Gemeinsamer Kontext: ':'Shared context: '))+hub.label);
    }
  }
  for(const edge of peerPairs.values())if(!links.some(l=>[l.source,l.target].includes(edge.source)&&[l.source,l.target].includes(edge.target)))links.push(edge);
  const relationLabel=l=>l.reasons?.join(' · ')||ui.relations[l.type]||l.type;
  ...
  function render(refit=false){
    ...
    if(mode==='map'&&selected&&byId.get(selected)?.type!=='book'){visible=visible.filter(n=>near.has(n.id));}
    ...
    if(mode==='map'){
      ...
      const sim=d3.forceSimulation(visible).randomSource(d3.randomLcg(.42)).force('x',d3.forceX(n=>n.px).strength(.04)).force('y',d3.forceY(n=>n.py).strength(.04)).force('collide',d3.forceCollide(n=>selected&&near.has(n.id)?65:29)).stop();for(let i=0;i<160;i++){sim.tick();for(const n of visible){n.x=Math.max(28,Math.min(width-28,n.x));n.y=Math.max(width<650?290:225,Math.min(height-295,n.y));}}
    ...
    const ids=new Set(visible.map(n=>n.id)),drawLinks=links.filter(l=>ids.has(l.source)&&ids.has(l.target)&&(!selected?(mode==='network'||(byId.get(l.source).type==='book'&&byId.get(l.target).type==='book')):l.source===selected||l.target===selected));
    const strokes=edgeLayer.selectAll('.library-edge').data(drawLinks).join('path').attr('class','library-edge').attr('data-source',l=>l.source).attr('data-target',l=>l.target).attr('d',l=>{const a=byId.get(l.source),b=byId.get(l.target),dx=b.x-a.x,dy=b.y-a.y;return 'M'+a.x+','+a.y+' Q'+((a.x+b.x)/2-dy*.12)+','+((a.y+b.y)/2+dx*.12)+' '+b.x+','+b.y;}).classed('is-active',!!selected).style('opacity',selected?1:.24);
    strokes.append('title').text(relationLabel);
    svg.classed('has-selection',!!selected);
    const g=nodeLayer.selectAll('g').data(visible,n=>n.id).join('g').attr('class','library-node').attr('role','button').attr('tabindex',0).attr('data-node',n=>n.id).attr('aria-label',n=>n.label+' · '+n.typeLabel+(n.language?' · '+n.language.toUpperCase():'' )).attr('aria-pressed',n=>String(n.id===selected)).attr('transform',n=>`translate(${n.x},${n.y})`).style('opacity',n=>selected&&!near.has(n.id)?.28:1).on('click',(e,n)=>{e.stopPropagation();if(n.type==='region'){region=n.region;updateRegions();render(true);}else select(n.id);}).on('keydown',(e,n)=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();if(n.type==='region'){region=n.region;updateRegions();render(true);}else select(n.id,true);}});
    ...
    g.append('text').attr('text-anchor','middle').attr('y',48).text(n=>near.has(n.id)||query||n.type!=='book'?(n.label.length>28?n.label.slice(0,26)+'…':n.label):'');
  ...
  const initial=new URLSearchParams(location.search).get('node');if(byId.has(initial)){selected=initial;if(byId.get(initial).type!=='book'){mode='network';document.querySelector('#library-map').setAttribute('aria-pressed','false');document.querySelector('#library-network').setAttribute('aria-pressed','true');}if(byId.get(initial).language)document.querySelector('#library-language').value=byId.get(initial).language;}show();
